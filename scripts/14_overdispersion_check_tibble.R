...
# Purpose: Save a tibble containing the mean and variance of the
# diverthours variable in each year to check for potential overdispersion.
...
# "11_littles_mcar_test.R", "12_missing_data_heatmap.R", and
# "13_diverthours_histogram.R".
...
# Create tibble
overdispersion_check_tibble <- ambulance %>%
  group_by(year) %>%
  summarize(meandiverthours = mean(diverthours),
            vardiverthours = var(diverthours))

# Save tibble
saveRDS(overdispersion_check_tibble,
        "tibbles/overdispersion_check_tibble.rds")